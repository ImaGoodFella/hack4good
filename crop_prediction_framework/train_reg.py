...
# Only select maize columns and relevant damage columns
#label_df = label_df[label_df['crop_name'] == 'maize']
#label_df = label_df[label_df['damage'].isin(['G', 'WD', 'DR'])]

# Setting the labels and join columns
label_df['is_damage'] = label_df['extent'] #(label_df['extent'] >= 20).astype(int)
#label_df['is_damage'] = label_df['damage']
#label_df.loc[label_df['is_damage'].isin(['ND', 'PS', 'DS', 'WN', 'FD']), 'is_damage'] = 'other'
...
train_loader, val_loader, test_loader = get_train_val_test_dataloaders(img_size=img_size, img_dir=img_dir, feature_df=feature_df, feature_columns=feature_columns,
                                                                       ...
                                                                       split_sizes=[0.8, 0.1, 0.1], batch_size=128, num_workers=64, random_state=42, map_labels=False)


# Define Model
num_classes = train_loader.dataset.num_classes
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = get_basic_model(num_classes=1, num_ts_features=len(feature_columns), device=device, use_multi_gpu=True)
...
criterion = torch.nn.MSELoss() #torch.nn.CrossEntropyLoss(weight=class_weights)
